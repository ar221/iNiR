...
Singleton {
    ...
    property var _singleWindowExpandedWorkspaces: ({})
    property int _singleWindowPendingWorkspaceId: -1
    property int _singleWindowPendingWindowId: -1
    property int _singleWindowPendingStage: 0
    property int _singleWindowPendingAttempts: 0
    readonly property int _singleWindowMaxAttempts: 6
    ...
    onWindowsChanged: scheduleSingleWindowFullWidthCheck()
    onFocusedWorkspaceIdChanged: scheduleSingleWindowFullWidthCheck()
    onActiveWindowChanged: scheduleSingleWindowFullWidthCheck()
    onCurrentLayoutChanged: scheduleSingleWindowFullWidthCheck()
    onSingleWindowFullWidthEnabledChanged: {
        if (!singleWindowFullWidthEnabled) {
            _singleWindowExpandedWorkspaces = ({})
            clearPendingSingleWindowExpand()
            return
        }

        scheduleSingleWindowFullWidthCheck()
    }
    ...
    Connections {
        ...
        function onActiveChanged(): void {
            root.scheduleSingleWindowFullWidthCheck()
        }
    }
    ...
    Timer {
        id: singleWindowFullWidthTimer
        interval: 140
        repeat: false
        onTriggered: root.evaluateSingleWindowFullWidth()
    }

    Timer {
        id: singleWindowExpandActionTimer
        interval: 140
        repeat: false
        onTriggered: root.processSingleWindowExpandAttempt()
    }
    ...
    function scheduleSingleWindowFullWidthCheck(): void {
        if (!CompositorService.isNiri)
            return

        singleWindowFullWidthTimer.restart()
    }

    function markWorkspaceSingleWindowExpanded(workspaceId, windowId): void {
        if (!workspaceId)
            return

        const nextState = Object.assign({}, root._singleWindowExpandedWorkspaces)

        if (windowId >= 0)
            nextState[workspaceId] = windowId
        else
            delete nextState[workspaceId]

        root._singleWindowExpandedWorkspaces = nextState
    }

    function expandedWindowIdForWorkspace(workspaceId): int {
        const value = root._singleWindowExpandedWorkspaces[workspaceId]
        return typeof value === "number" ? value : -1
    }

    function clearPendingSingleWindowExpand(): void {
        root._singleWindowPendingWorkspaceId = -1
        root._singleWindowPendingWindowId = -1
        root._singleWindowPendingStage = 0
        root._singleWindowPendingAttempts = 0
        singleWindowExpandActionTimer.stop()
    }
    ...
    function isSingleWindowPolicyBlocked(workspaceId, tilingWins): bool {
        if (GameMode.active || root.currentLayout !== "off")
            return true

        const workspace = root.workspaces[workspaceId]
        if (!workspace?.is_focused)
            return true

        const focusedWindow = root.activeWindow
        if (!focusedWindow || focusedWindow.workspace_id !== workspaceId || focusedWindow.is_floating)
            return true

        if (root.isWindowFullscreenLikeForAutoExpand(focusedWindow))
            return true

        return tilingWins.some(window => root.isWindowFullscreenLikeForAutoExpand(window))
    }

    function armPendingSingleWindowExpand(workspaceId, windowId): void {
        const changedTarget = root._singleWindowPendingWorkspaceId !== workspaceId
            || root._singleWindowPendingWindowId !== windowId

        if (changedTarget) {
            root._singleWindowPendingWorkspaceId = workspaceId
            root._singleWindowPendingWindowId = windowId
            root._singleWindowPendingStage = 0
            root._singleWindowPendingAttempts = 0
        }

        singleWindowExpandActionTimer.restart()
    }

    function processSingleWindowExpandAttempt(): void {
        const workspaceId = root._singleWindowPendingWorkspaceId
        const windowId = root._singleWindowPendingWindowId

        if (workspaceId < 0 || windowId < 0)
            return

        const focusedWs = root.allWorkspaces.find(ws => ws.is_focused)
        if (!focusedWs || focusedWs.id !== workspaceId) {
            root.clearPendingSingleWindowExpand()
            return
        }

        const tilingWins = root.windows.filter(w => w.workspace_id === workspaceId && !w.is_floating)
        if (tilingWins.length !== 1 || tilingWins[0].id !== windowId || root.isSingleWindowPolicyBlocked(workspaceId, tilingWins)) {
            root.markWorkspaceSingleWindowExpanded(workspaceId, -1)
            root.clearPendingSingleWindowExpand()
            return
        }

        if (root.expandedWindowIdForWorkspace(workspaceId) === windowId) {
            root.clearPendingSingleWindowExpand()
            return
        }

        if (root._singleWindowPendingAttempts >= root._singleWindowMaxAttempts) {
            root.clearPendingSingleWindowExpand()
            return
        }

        if (root.activeWindow?.id !== windowId) {
            root._singleWindowPendingStage = 0
            root._singleWindowPendingAttempts += 1
            root.focusWindow(windowId)
            singleWindowExpandActionTimer.restart()
            return
        }

        if (root._singleWindowPendingStage === 0) {
            root._singleWindowPendingStage = 1
            singleWindowExpandActionTimer.restart()
            return
        }

        root.expandColumnToAvailableWidth()
        root.markWorkspaceSingleWindowExpanded(workspaceId, windowId)
        root.clearPendingSingleWindowExpand()
    }

    function evaluateSingleWindowFullWidth(): void {
        ...
        const workspaceId = focusedWs.id
        const tilingWins = root.windows.filter(w => w.workspace_id === workspaceId && !w.is_floating)

        if (root.isSingleWindowPolicyBlocked(workspaceId, tilingWins)) {
            root.markWorkspaceSingleWindowExpanded(workspaceId, -1)
            if (root._singleWindowPendingWorkspaceId === workspaceId)
                root.clearPendingSingleWindowExpand()
            return
        }

        if (tilingWins.length !== 1) {
            const expandedId = root.expandedWindowIdForWorkspace(workspaceId)
            if (expandedId >= 0) {
                // Previously expanded window needs to give up space for new windows.
                // Focus it and reset to preset column width so Niri redistributes evenly.
                const expandedWin = tilingWins.find(w => w.id === expandedId)
                if (expandedWin && root.activeWindow?.id === expandedId)
                    root.switchPresetColumnWidth()
            }
            root.markWorkspaceSingleWindowExpanded(workspaceId, -1)
            if (root._singleWindowPendingWorkspaceId === workspaceId)
                root.clearPendingSingleWindowExpand()
        ...
        const targetWindow = tilingWins[0]
        if (root.expandedWindowIdForWorkspace(workspaceId) === targetWindow.id) {
            if (root._singleWindowPendingWorkspaceId === workspaceId && root._singleWindowPendingWindowId === targetWindow.id)
                root.clearPendingSingleWindowExpand()
            return
        }

        root.markWorkspaceSingleWindowExpanded(workspaceId, -1)
        root.armPendingSingleWindowExpand(workspaceId, targetWindow.id)
    }
    ...
    function _workspaceEquivalent(a, b): bool {
        if (!a || !b)
            return false
    ...
    function _refreshWorkspaceDerivedStateFromMap(workspaceMap): void {
        const sorted = Object.values(workspaceMap).sort((a, b) => a.idx - b.idx)
        root.allWorkspaces = sorted

        root.focusedWorkspaceIndex = sorted.findIndex(w => w.is_focused)
        if (root.focusedWorkspaceIndex >= 0) {
            const focusedWs = sorted[root.focusedWorkspaceIndex]
            root.focusedWorkspaceId = focusedWs.id
            root.currentOutput = focusedWs.output || ""
        } else {
            root.focusedWorkspaceIndex = 0
            root.focusedWorkspaceId = ""
        }

        root.updateCurrentOutputWorkspaces()
    }

    function _setWorkspaceActiveWindowId(workspaceId, activeWindowId): void {
        ...
        updatedWorkspaces[workspaceId] = Object.assign({}, ws, { active_window_id: activeWindowId })
    ...
    function handleWorkspacesChanged(data) {
        ...
        for (const ws of data.workspaces) {
            ...
            const candidate = Object.assign({}, ws)
            if (oldWs && oldWs.active_window_id !== undefined && candidate.active_window_id === undefined)
                candidate.active_window_id = oldWs.active_window_id

            if (oldWs && root._workspaceEquivalent(oldWs, candidate))
                newWorkspaces[ws.id] = oldWs
            else {
                newWorkspaces[ws.id] = candidate
                hasChanges = true
            }
        }

        if (!hasChanges && Object.keys(root.workspaces).length === data.workspaces.length)
            return

        root.workspaces = newWorkspaces
        root._refreshWorkspaceDerivedStateFromMap(newWorkspaces)
    }
    ...
    function handleWorkspaceActivated(data) {
        ...
        root.workspaces = updatedWorkspaces
        root._refreshWorkspaceDerivedStateFromMap(updatedWorkspaces)
    }
    ...
    function handleWindowFocusChanged(data) {
        ...
        if (focusedWindowId !== null && focusedWindowId !== undefined) {
            if (mruWindowIds[0] !== focusedWindowId) {
                const newOrder = []
        ...
        let focusedWindow = nextFocusedIndex >= 0 ? windows[nextFocusedIndex] : null

        if (previousFocusedIndex !== nextFocusedIndex) {
            const updatedWindows = windows.slice()
            if (previousFocusedIndex >= 0) {
                const unfocused = Object.assign({}, updatedWindows[previousFocusedIndex])
                unfocused.is_focused = false
                updatedWindows[previousFocusedIndex] = unfocused
            }
            if (nextFocusedIndex >= 0) {
                const focused = Object.assign({}, updatedWindows[nextFocusedIndex])
                focused.is_focused = true
                updatedWindows[nextFocusedIndex] = focused
        ...
        if (focusedWindow) {
            root._setWorkspaceActiveWindowId(focusedWindow.workspace_id, focusedWindowId)
        }
    }

    function handleWorkspaceActiveWindowChanged(data) {
        root._setWorkspaceActiveWindowId(data.workspace_id, data.active_window_id)

        const targetWindowId = data.active_window_id
        if (targetWindowId !== null && targetWindowId !== undefined) {
            ...
            let newActiveWindow = targetIndex >= 0 ? windows[targetIndex] : null
            if (previousFocusedIndex !== targetIndex) {
                const updatedWindows = windows.slice()
                if (previousFocusedIndex >= 0) {
                    const unfocused = Object.assign({}, updatedWindows[previousFocusedIndex])
                    unfocused.is_focused = false
                ...
                if (targetIndex >= 0) {
                    const focused = Object.assign({}, updatedWindows[targetIndex])
                    focused.is_focused = true
                    updatedWindows[targetIndex] = focused
                    newActiveWindow = focused
                }
                windows = updatedWindows
            }
        ...
        for (let i = 0; i < windows.length; i++) {
            const w = windows[i]
            const shouldBeFocused = (w.workspace_id == data.workspace_id) ? false : w.is_focused
    ...
    function handleWorkspaceUrgencyChanged(data) {
        ...
        if (!ws)
            return
        if (ws.is_urgent === data.urgent)
            return

        const updatedWs = Object.assign({}, ws, { is_urgent: data.urgent })
        const updatedWorkspaces = Object.assign({}, root.workspaces)
        updatedWorkspaces[data.id] = updatedWs
        root.workspaces = updatedWorkspaces
