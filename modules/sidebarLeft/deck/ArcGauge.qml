 ...
 *
 * Hot threshold at `hotThreshold` (default 0.8) — fill goes #ff3333.
 * Temperature colouring: ≤50°C colPrimary → 70°C colTertiary → ≥85°C #ff3333.
 */
Item {
    id: root
    ...
    // ── Temperature colour ─────────────────────────────────────────────
    // ≤50°C → colPrimary, 50–70 → lerp to colTertiary, 70–85 → lerp to #ff3333, ≥85 → #ff3333
    readonly property color _tempColor: {
        ...
        if (t <= 50) return cool
        if (t >= 85) return Qt.color("#ff3333")
        if (t <= 70) {
            const f = (t - 50) / 20
            return Qt.rgba(
        ...
        const hot = Qt.color("#ff3333")
        const f = (t - 70) / 15
        return Qt.rgba(
            warm.r + (hot.r - warm.r) * f,
            warm.g + (hot.g - warm.g) * f,
            warm.b + (hot.b - warm.b) * f,
            1)
    }
